...
var options = {
  url:
    "https://app.oddsapi.io/api/v1/odds?sport=american-football&country=usa&league=american-football-usa-nfl&apikey=ccdd7160-f5df-11e9-9e1c-499eb4566c8b",
  headers: headers
};
...
function gameCallback(error, response, body) {
  if (!error && response.statusCode == 200) {
    processGameJSON(body);
  } else {
    console.log("Error posting scrapped data to DB:");
    console.log(error);
    console.log(response);
...
function processGameJSON(js) {
  ...
  for (var item of j_data) {
    ...
    request(options2, function(error, res, b) {
      if (!error && res.statusCode == 200) {
        console.log("added game to db successfully");
      }
    });
  }
}
...
exports.getGameFromAPI = function() {
  request(options, gameCallback);
};
