...
exports.dbAddEntry = function(request, response) {
    ...
    var teamsTag = request.body["Teams"].sort().join('').toLowerCase();
    
    var querry   = {"Teams"          : {$eq  : teamsTag},
...
exports.dbGetData = function(request, response) {
    var querry = {"Teams" : {"$eq" : [request.query.teama, request.query.teamb].sort().join("").toLowerCase()},
...
exports.dbGetDataSite = function(request, response) {
    var querry = {"Teams" : {"$all" : [request.query.teama, request.query.teamb].sort().join("").toLowerCase()},
                  "BettingFormat": {"$eq" : request.query.datatype}};
    BDPointSchema.find(querry)
	.exec()
	.then((res) => {
